...
def get_tg_user(request: HttpRequest):
    ...
    print(f"🔍 get_tg_user(): session telegram_id={tg_id} (type: {type(tg_id)})")

    if not tg_id:
        return None
    try:
        user = TelegramUser.objects.get(telegram_id=tg_id)
        print(f"🔍 get_tg_user(): found user={user.username}")
        return user
    except TelegramUser.DoesNotExist:
        print(f"🔍 get_tg_user(): user not found for telegram_id={tg_id}")
        return None
